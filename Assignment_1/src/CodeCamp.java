 ...
 *
 *  On my honor, RAHIK N SIKDER, this programming assignment is my own 
...
public class CodeCamp {
    ...
    public static boolean isPermutation(int[] aData, int[] bData) {
        ...
        for (int i = 0; i < aData.length; i++) {
            int element = aData[i];
            int frequency = findFrequency(element, aData);
            if (frequency != findFrequency(element, bData)) {
                return false;
            }
        }
     ...
     * 'U', and 'u'</tt>.
     * The parameter <tt>arrayOfStrings</tt> is not altered as a result of this
     * method.
     * <p>
    ...
    public static int mostVowels(String[] arrayOfStrings) {
        ...
        for (int i = 0; i < arrayOfStrings.length; i++) {
            ...
            // var created to avoid multiple checks
            boolean isNull = arrayOfStrings[i] == null;

            // null Strings stay -1, empty strings become 0
            int numVowels = -1;
            ...
            if (!isNull) {
                ...
                numVowels = arrayOfStrings[i].length() - str.length();
            }
    ...
    public static int sharedBirthdays(int numPeople, int numDaysInYear) {
        ...
        int[] freqPerDay = new int[numDaysInYear];
        int bDayPairs = 0;
        for (int i = 0; i < numPeople; i++) {
            int curBday = (int) (Math.random() * numDaysInYear);
            if (freqPerDay[curBday] >= 1) {
                bDayPairs += freqPerDay[curBday];
            }
            freqPerDay[curBday]++;
        }
    ...
    public static boolean queensAreSafe(char[][] board) {
        ...
        for (int row = 0; row < board.length; row++) {
            for (int col = 0; col < board[row].length; col++) {
                ...
                // At a queen, check if another queen lies in its vision
                if (board[row][col] == 'q') {
                    boolean queenIsSafe = 
                            checkRowandCol(row, col, board)
    ...
    // Checks the row and column for queens
    private static boolean checkRowandCol(int row, int col, char[][] board) {
        int numQueens = 0;
    ...
    // Checks diagonals for queens
    private static boolean checkDiagonals(int row, int col, char[][] board) {

        // LEFT TO RIGHT DIAGONAL

        // Checking up the diagonal
        int curColumn = col - 1;
        for (int curRow = row - 1; curRow >= 0 && curColumn >= 0; curRow--) {
            if (board[curRow][curColumn] == 'q') {
                return false;
            }
            curColumn--;
        }

        // Checking down the diagonal
        curColumn = col + 1;
        for (int curRow = row + 1; curRow < board.length && curColumn < board.length; curRow++) {
            if (board[curRow][curColumn] == 'q') {
                return false;
            }
            curColumn++;
        ...
        // Checking up the diagonal
        curColumn = col + 1;
        for (int curRow = row - 1; curRow >= 0 && curColumn < board.length; curRow--) {
            if (board[curRow][curColumn] == 'q') {
                return false;
            }
            curColumn++;
        ...
        for (int curRow = row + 1; curRow < board.length && curColumn >= 0; curRow++) {
            if (board[curRow][curColumn] == 'q') {
                return false;
            }
            curColumn--;
    ...
    public static int getValueOfMostValuablePlot(int[][] city) {
        ...
        int maxValOfPlot = city[0][0];
        // Accessing every possible upper left corner
        for (int row = 0; row < city.length; row++) {
            for (int col = 0; col < city[row].length; col++) {
                // Find all possible lower corners from current position
                for (int row2 = row; row2 < city.length; row2++) {
                    for (int col2 = col; col2 < city[row2].length; col2++) {
                        int[] upperLeftCorner = { row, col };
                        int[] lowerRightCorner = { row2, col2 };
                        int curVal = findValOfSubPlot(city, upperLeftCorner, lowerRightCorner);
                        maxValOfPlot = (curVal > maxValOfPlot) ? curVal : maxValOfPlot;
                    }
                }
    ...
    public static int findValOfSubPlot(int[][] city, int[] upLeftCorner, int[] lowRightCorner) {
        int val = 0;
        for (int row = upLeftCorner[0]; row <= lowRightCorner[0]; row++) {
            for (int col = upLeftCorner[1]; col <= lowRightCorner[1]; col++) {
                val += city[row][col];
            }
        }
        return val;
    ...
    public static void runExperiment1() {
        ...
        for (int i = 0; i < NUM_EXPERIMENTS; i++) {
            int pairs = sharedBirthdays(NUM_PEOPLE, DAYS_IN_YEAR);
            System.out.println("Run " + i + ": " + pairs);
            totalPairs += pairs;
        }

        int avg = totalPairs / NUM_EXPERIMENTS;
        System.out.println("Average " + avg);

    }

    public static void runExperiment2() {
        final int NUM_EXPERIMENTS = 50000;
        final int DAYS_IN_YEAR = 365;
        final int NUM_PEOPLE_START = 2;
        final int NUM_PEOPLE_END = 100;

        for (int numPeople = NUM_PEOPLE_START; numPeople <= NUM_PEOPLE_END; numPeople++) {
            int atLeastOneBday = 0;
            for (int i = 0; i < NUM_EXPERIMENTS; i++) {
                int pairs = sharedBirthdays(numPeople, DAYS_IN_YEAR);
                if (pairs >= 1) {
                    atLeastOneBday++;
                }
            }
            double percentage = (double) atLeastOneBday / NUM_EXPERIMENTS;
            percentage *= 100;

            System.out.println("Num people: " + numPeople
                    + ", number of experiments with one or more shared birthday: "
                    + atLeastOneBday + ",\tpercentage: " + String.format("%.2f", percentage));
