...
public class Graph {
    ...
    public void dijkstra(String startName) {
        ...
        // Get the start vertex and set num edges to 0
        Vertex startVertex = vertices.get(startName);
        startVertex.numEdgesFromStartVertex = 0;

        // Create a priority queue with all the edges from start and put start into it
        PriorityQueue<Path> vertexQueue = new PriorityQueue<>();
        vertexQueue.add(new Path(startVertex, 0));

        // While there are vertices in the queue...
        while (!vertexQueue.isEmpty()) {

            Path curPath = vertexQueue.poll();
            Vertex curVertex = curPath.dest;

            if (curVertex.scratch == 0) {
                // Mark vertex as visited and update cost of vertex
                curVertex.scratch = 1;
                curVertex.weightedCostFromStartVertex = curPath.weightedCost();

                // Want to go through every edge and add a new Paths to queue as needed
                for (Edge e : curVertex.adjacent) {

                    Vertex dest = e.dest;
                    double newCost = curPath.weightedCost() + e.cost;

                    if (newCost < dest.weightedCostFromStartVertex) {
                        // Update cost, num edges from start, and prev
                        dest.weightedCostFromStartVertex = newCost;
                        dest.numEdgesFromStartVertex = curVertex.numEdgesFromStartVertex + 1;
                        dest.prev = curVertex;
                        if(curVertex.name.equals("F"))
                            System.out.println("About to add " + dest.name + " for " + newCost);
                        vertexQueue.add(new Path(dest, newCost));
                    }
                }
            }
        }
     ...
     * <br>
     * <br>
     * if weighted == true, use dijkstra, otherwise, use unwieghted
     * shortest path
     * 
     * <br>
     * <br>
    ...
    public void findAllPaths(boolean weighted) {
        ...
        // Want to initialize the longest path and reset vertex totals
        longest = new Path();
        ...
        // Want to access every single vertex in the graph
        for (String vertexKey : vertices.keySet()) {

            Vertex vertex = vertices.get(vertexKey);

            // Want to consider this our starting vertex, from where the following dests
            // will be updated with the path lengths
            if (weighted) {
                dijkstra(vertexKey);
            } else {
                findUnweightedShortestPath(vertexKey);
            }

            // Want to see all the edges and adjacent vertices with this Vertex
            for (String destKey : vertices.keySet()) {
                Vertex dest = vertices.get(destKey);

                if (!destKey.equals(vertexKey) && dest.weightedCostFromStartVertex != INFINITY) {

                    // There exists a path!
                    vertex.numVertexConnected++;

                    // Both should be equal when weighted == false
                    vertex.totalUnweightedPathLength += dest.numEdgesFromStartVertex;
                    vertex.totalWeightedPathLength += dest.weightedCostFromStartVertex;

                    // Update longest if applicable. If statement works for both weighted and not
                    if (dest.weightedCostFromStartVertex > longest.weightedCostOfPath) {

                        longest.weightedCostOfPath = dest.weightedCostFromStartVertex;
                        longest.verticesInPath = new LinkedList<>();
                        List<String> paths = findPath(destKey);
                        for (String p : paths) {
                            longest.verticesInPath.add(vertices.get(p));
                        }
